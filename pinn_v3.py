"""
Band-agnostic real-space inverse design (v3).

Builds on pinn_v2: real-space sigmoid+C4v parameterization, multi-target
training with soft gap selection across all band pairs.

New in v3:
    --gap-mode ratio|width  selects between gap/midgap ratio and raw gap width.

Usage:
    python pinn_v3.py --steps 500 --lr 1e-3
    python pinn_v3.py --steps 500 --gap-mode width
    python pinn_v3.py eval --checkpoint pinn_v3_model.pt
"""
...
class FourierFeatureEmbedding(nn.Module):
    """Positional encoding: scalar f -> [f, sin(2*pi*f), cos(2*pi*f), ...]."""
...
# ===================================================================
# 3. InverseDesignNetV3
# ===================================================================

class InverseDesignNetV3(nn.Module):
...
def train(cfg):
    ...
    print(f"Inverse Design v3 | Grid: {cfg.n_grid} | Steps: {cfg.steps}")
...
def _plot_training(results, history, eval_history, k_dist, tick_pos, tick_labels, cfg):
    ...
    fig.tight_layout()
    fig.savefig("pinn_v3_training.png", dpi=150)
    print("Saved pinn_v3_training.png")
    plt.close(fig)
...
def _plot_evaluation_grid(results, k_dist, tick_pos, tick_labels, cfg):
    ...
    fig.tight_layout()
    fig.savefig("pinn_v3_eval.png", dpi=150)
    print("Saved pinn_v3_eval.png")
    plt.close(fig)
...
def _plot_interpolation(model, cfg, m_indices, g_vectors, k_points,
    ...
    fig.tight_layout()
    fig.savefig("pinn_v3_interp.png", dpi=150)
    print("Saved pinn_v3_interp.png")
    plt.close(fig)
...
def load_model(checkpoint="pinn_v3_model.pt"):
...
def _add_train_args(parser):
    ...
    parser.add_argument("--gap-mode", choices=["ratio", "width"], default="ratio",
                        help="'ratio' maximizes gap/midgap; 'width' maximizes raw gap")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ("train", "eval"):
        p = argparse.ArgumentParser(description="Band-agnostic real-space inverse design v3")
        sub = p.add_subparsers(dest="command")
        tr = sub.add_parser("train")
        _add_train_args(tr)
        ev = sub.add_parser("eval")
        ev.add_argument("--checkpoint", type=str, default="pinn_v3_model.pt")
