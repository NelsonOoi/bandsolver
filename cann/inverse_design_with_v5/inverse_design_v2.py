"""
Inverse design v2: periodic geometry enforcement.

Changes from v1:
  - NST uses Fourier-space parameterization (irfft2 -> periodic by construction)
  - RBF uses minimum-image periodic wrapping for unit cell continuity
  - Auto band-gap detection (smooth_max over all adjacent pairs)
  - Bands clamped non-negative after unstandardization
  - Relative gap objective (gap/midgap) for scale-invariant dual-physics loss

Usage:
    python inverse_design_v2.py
"""
...
class MaskRasterizer(nn.Module):
    ...
    def __init__(self, n_rbf_side: int = 6, grid_size: int = 32,
        ...
        diff = diff - torch.round(diff)  # periodic wrapping
...
def optimize_nst(model, stats, specs, n_starts=32, n_steps=500,
    ...
    G = GRID_SIZE
    z_freq = torch.randn(n_starts, G, G // 2 + 1, 2, device=device) * 0.1
    z_freq = nn.Parameter(z_freq)
    optimizer = torch.optim.Adam([z_freq], lr=lr)
    ...
    for step in range(n_steps):
        ...
        z_complex = torch.view_as_complex(z_freq)
        z = torch.fft.irfft2(z_complex.cpu(), s=(G, G)).to(z_freq.device)
        ...
        with torch.no_grad():
            z_snap = torch.fft.irfft2(
                torch.view_as_complex(z_freq).cpu(), s=(G, G)).to(z_freq.device)
            curr_mask = torch.sigmoid(beta_final * z_snap)
...
def main():
    ...
    plot_result(results, meta, specs, save_path="inverse_design_v2_result.png")
    print("\nDone.")
